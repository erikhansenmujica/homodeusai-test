...
`src/decide.ts` orchestrates non-bypassable stages. Conversation context, answer sufficiency/shape, versioned domain configuration, retrieval, governance, and conflict detection are separate modules. Up to three completed user turns may supply topic context; assistant text is never evidence or trusted requester context. The raw user question always runs as the primary semantic query. A high-margin concept may add recall, but a low-absolute-score expansion normally requires the direct top source type to be preferred. FAQ concepts and declared multi-source conflict checks may instead rely on a domain-aligned preferred source found in both the top 12 corpus-derived lexical candidates and the calibrated direct semantic near-top window; neither path can establish direct prompt support or escape a known-gap boundary. Strong direct FAQ evidence also defeats a cross-domain concept, while policy and agreement passages can still use concept expansion for recall. Direct-question and retrieval-concept scores remain separate so a uniquely aligned concept passage can break a close platform-sensitive tie without replacing raw-query relevance. FAQ prompt and answer vectors provide two additional independent views of each row; one selection margin absorbs harmless native-versus-x64 score drift. Live individual state still requires a live authorized record, and static FAQ evidence must have strong direct coverage or lexical corroboration before it can be treated as capability guidance. Every material clause must independently match an eligible passage and requested answer shape; compound selection maximizes a complementary cross-source pair rather than accepting the first two matches. Lexical-only support requires a 60% subject-term coverage floor; learned support requires calibrated topic and answer-vector alignment. A direct answer is copied from exact eligible passages; every material body statement is also a claim.
...
- Conflict detection is deliberately conservative. Versioned semantic concepts identify questions that require consensus across eligible authorities; consensus candidates must align with both the direct question and the selected conflict concept, and a typed consensus must include every configured authority class. This prevents unrelated same-domain passages or two same-class FAQ matches from becoming a false policy conflict. Protected-source concepts must win their classifier and support an explicitly compatible answer shape before early policy-sensitive routing. If every otherwise relevant source rejects the same trusted profile, the route remains `profile_mismatch` even when those sources also have audience restrictions. Unicode numeric comparison adds a deterministic language-neutral signal. In degraded mode, unresolved multi-authority ambiguity abstains. This is not a general natural-language theorem prover.
