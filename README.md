...
The finished desk preserves the locked API and browser hooks while adding truthful runtime readiness, checksum-pinned multilingual semantic retrieval, exact extractive evidence, three-turn user-only follow-up context, durable idempotent handoffs, bounded redacted traces, metrics/logging, accessible internal source navigation, and real Playwright coverage. The user’s question is always retrieved directly. A confidently selected, versioned semantic concept may add a second retrieval query, but a low-score expansion normally requires the direct top source type to be preferred. FAQ concepts and declared multi-source conflict checks may instead use a domain-aligned preferred source found in both the top 12 corpus-derived lexical candidates and the calibrated direct semantic near-top window; strong direct FAQ evidence still rejects an unrelated cross-domain concept. FAQ prompts and answers are embedded separately, and a hint cannot override a known corpus gap without directly corroborated question support. Concept-based conflicts must remain aligned with the concept passage set and every configured authority class. There are no query-language synonym or keyword tables; lexical BM25 remains a corpus-derived corroboration and safe-degradation signal.

The authored evaluation has **21 cases** (12 answers, 8 deferrals, 1 conversation). A separate **63-question adversarial regression matrix** covers conflicts, profile boundaries, indirect references, compound live-state questions, restricted information, injection, paraphrases, and service routing; a further **20-question open-world suite** contains supported questions that are absent from routing configuration, including the eight presentation-probe regressions. The lexical-only answer gate requires at least **60% subject-term coverage**; the learned path may instead satisfy calibrated semantic topic and answer alignment while all governance gates remain deterministic. The final fresh-state local matrix on 2026-07-25 measured degraded readiness/first decision at **1.078 s/131 ms** and learned at **52.95 s/263 ms**. The final restricted image, with its prebuilt index, reached `ready_learned` in **13.42 s** and returned its first end-to-end decision in **1.351 s**, below the 10-second container ceiling. Learned mode passed **21/21**; degraded mode matched 9/21 exact outcomes and safely abstained on the remainder, with no unsupported autonomous answer. The adversarial suite passed **63/63** on both native and Linux/x64 learned runtimes, and the open-world suite passed **20/20**. The matrix therefore selects learned retrieval as the default. Provider state is reported as `ok` or `degraded`.
