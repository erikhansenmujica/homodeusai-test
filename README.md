...
The finished desk preserves the locked API and browser hooks while adding truthful runtime readiness, checksum-pinned multilingual semantic retrieval, exact extractive evidence, three-turn user-only follow-up context, durable idempotent handoffs, bounded redacted traces, metrics/logging, accessible internal source navigation, and real Playwright coverage. Versioned semantic prototypes replace query-language synonym and keyword tables; lexical BM25 remains a corpus-derived corroboration and safe-degradation signal.

The authored evaluation has **21 cases** (12 answers, 8 deferrals, 1 conversation). The lexical-only answer gate requires at least **60% subject-term coverage**; the learned path may instead satisfy calibrated semantic topic and answer alignment while all governance gates remain deterministic. The final local matrix measured degraded readiness/first decision at **1.03 s/125 ms** and learned at **4.59 s/181 ms**. Learned mode passed **21/21**; degraded mode matched 9/21 exact outcomes and safely abstained on the remainder, with no unsupported autonomous answer. The restricted container reached learned readiness in **20.58 s** and returned its first decision in **0.93 s**. The matrix therefore selects learned retrieval as the default. Provider state is reported as `ok` or `degraded`.
